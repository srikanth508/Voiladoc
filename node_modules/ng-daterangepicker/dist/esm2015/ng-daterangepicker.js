import { Component, HostListener, ElementRef, forwardRef, Input, NgModule } from '@angular/core';
import { NG_VALUE_ACCESSOR, FormsModule } from '@angular/forms';
import { startOfDay, startOfMonth, endOfMonth, eachDay, getDate, getDay, isToday, isFirstDayOfMonth, isLastDayOfMonth, isSameDay, isWithinRange, subDays, format, isAfter, isBefore, subMonths, addMonths, subWeeks, startOfWeek, endOfWeek, startOfYear, endOfYear, subYears } from 'date-fns';
...
class NgDateRangePickerComponent {
    /**
     * @param {?} elementRef
     */
    ...
    generateCalendar() {
        this.days = [];
        let /** @type {?} */ start = startOfMonth(this.date);
        let /** @type {?} */ end = endOfMonth(this.date);
        let /** @type {?} */ days = eachDay(start, end).map(d => {
            return {
                date: d,
                day: getDate(d),
                weekday: getDay(d),
        ...
        let /** @type {?} */ prevMonthDayNum = getDay(start) == 0 ? 6 : getDay(start) - 1;
        let /** @type {?} */ prevMonthDays = [];
        if (prevMonthDayNum > 0) {
            prevMonthDays = Array.from(Array(prevMonthDayNum).keys()).map(i => {
                let /** @type {?} */ d = subDays(start, prevMonthDayNum - i);
                return {
                    date: d,
                    day: getDate(d),
                    weekday: getDay(d),
        ...
        this.value = `${format(this.dateFrom, this.options.outputFormat)}-${format(this.dateTo, this.options.outputFormat)}`;
    }
    /**
     * @param {?} e
     * @param {?} selection
    ...
    selectDate(e, index) {
        e.preventDefault();
        let /** @type {?} */ selectedDate = this.days[index].date;
        if ((this.opened === 'from' && isAfter(selectedDate, this.dateTo)) ||
            (this.opened === 'to' && isBefore(selectedDate, this.dateFrom))) {
            return;
        }
        if (this.opened === 'from') {
     ...
     */
    selectRange(range) {
        let /** @type {?} */ today = startOfDay(new Date());
        switch (range) {
            case 'tm':
            ...
            case 'lw':
                ...
                this.dateFrom = startOfWeek(today, { weekStartsOn: this.options.startOfWeek });
                this.dateTo = endOfWeek(today, { weekStartsOn: this.options.startOfWeek });
                break;
            case 'tw':
                this.dateFrom = startOfWeek(today, { weekStartsOn: this.options.startOfWeek });
                this.dateTo = endOfWeek(today, { weekStartsOn: this.options.startOfWeek });
     ...
     */
    handleBlurClick(e) {
        let /** @type {?} */ target = e.srcElement || e.target;
...
NgDateRangePickerComponent.decorators = [
    { type: Component, args: [{
                ...
                template: `<div class="ng-daterangepicker"
...
/** @nocollapse */
NgDateRangePickerModule.ctorParameters = () => [];
...
export { NgDateRangePickerModule, DATERANGEPICKER_VALUE_ACCESSOR, NgDateRangePickerComponent };
//# sourceMappingURL=ng-daterangepicker.js.map
