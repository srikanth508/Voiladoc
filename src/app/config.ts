export default {
    // Set this to the base URL of your sample server, such as 'https://your-app-name.herokuapp.com'.
    // Do not include the trailing slash. See the README for more information:
    // SAMPLE_SERVER_BASE_URL: 'https://amazintchtokbox.herokuapp.com',
     SAMPLE_SERVER_BASE_URL:'https://voilodocopentokbox.herokuapp.com',
     Sessionurl:'https://14.192.17.225/MeridionalWebAPI/Doctor/genratevideosessionID',
    // Sessionurl:'https://14.192.17.225/VoilaDocWebAPI/Doctor/genratevideosessionID',
  //  Sessionurl:'http://localhost:4199/Doctor/genratevideosessionID',
    // OR, if you have not set up a web server that runs the learning-opentok-php code,
    // set these values to OpenTok API key, a valid session ID, and a token for the session.
    // For test purposes, you can obtain these from https://tokbox.com/account.
    API_KEY: '46927194',
    SESSION_ID: '',
    TOKEN: '',
    SECRET: 'c8c6a721d572a389b30da0c9195b80cab054772c'
  };
  

  // API_KEY: '46448242',
  // SESSION_ID: '',
  // TOKEN: '',
  // SECRET: 'e68d1950f23695f0300f0544d6d16cc91ffe655b'