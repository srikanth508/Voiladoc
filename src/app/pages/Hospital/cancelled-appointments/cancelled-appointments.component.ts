...
export class CancelledAppointmentsComponent implements OnInit {
  ...
  //   }).then((result) => {
  //     if (result.value) {
  //       this.docservice.CancelBookAppointmentWeb(id).subscribe(res => {
  //         let test = res;
  //         this.getbookappointmentbyhospitalbyhospitalid();
  //       })
  //       Swal.fire(
  ...
  public Refund() {
    this.docservice.CancelBookAppointmentWebRefund(this.appointmentID, this.paidAmount1).subscribe(res => {
      let test = res;
      Swal.fire("Refunded Amount Successfully");
      this.getbookappointmentbyhospitalbyhospitalid();
    })
  ...
  public getglmasterexcel() {
    ...
    this.exportAsExcelFile(hhh, "Patient Registeration List");
  }
