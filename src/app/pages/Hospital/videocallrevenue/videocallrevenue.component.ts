...
@Component({
  selector: 'app-videocallrevenue',
  templateUrl: './videocallrevenue.component.html',
  styleUrls: ['./videocallrevenue.component.css']
})
export class VideocallrevenueComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.docservice.GetHospitalAppointmentDetails(this.hospitalid, this.sdate, this.edate).subscribe(
      data => {
        ...
        this.dummlist = data;
        this.cancelledlist = this.dummlist.filter(x => x.appointmentTypeID == 2 && x.isVisited == 1);
